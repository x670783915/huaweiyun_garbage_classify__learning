...
class Crawler:
    ...
    def get_referrer(self, url):
        ...
        # 保存图片
    def save_image(self, rsp_data, path):
        if not os.path.exists("./" + path):
            os.mkdir("./" + path)
        # 判断名字是否重复，获取图片长度
        self.__counter = len(os.listdir('./' + path)) + 1
        for image_info in rsp_data['imgs']:
            ...
            try:
                ...
                # 保存图片
                urllib.request.urlretrieve(image_info['objURL'], './' + path + '/' + str(self.__counter) + str(suffix))
                ...
                print(path + "已有" + str(self.__counter) + "张小黄图")
                self.__counter += 1
        return

    # 开始获取
    def get_images(self, word='美女', path='downloads'):
    ...
    def start(self, word, path, spider_page_num=1, start_page=1):
        """
        爬虫入口
        :param word: 抓取的关键词
        ...
        self.get_images(word, path)


if __name__ == '__main__':
    crawler = Crawler(0.01)  # 抓取延迟为 0.05
    ...
    for index, values in json_data.items():
        ...
        if int(index) == 39:
            crawler.start(str(keyword), 'downloads/' + str(index), 4, 2)
        else:
            crawler.start(str(keyword), 'downloads/' + str(index), 2, 1)
