...
__all__ = ['get_mean_and_std', 'save_model', 'init_params', 'mkdir_p', 'AverageMeter', 'get_optimizer', 'save_checkpoint', 'mkdir']
...
def get_optimizer(model, train_layers, optim_name='sgd', lr=1e-3):
    tot_params = 0
    for name, params in model.named_parameters():
        ...
        if name in train_layers:
            ...
            tot_params += params.numel()
    # print(parameters)
    print(' Total params: %.2fM' % (tot_params / 1000000.0))
    if optim_name == 'sgd':
        return optim.SGD(
            parameters,
            lr=lr,
...
def save_model(state, is_best, checkpoint='checkpoint', model_name='testmodel'):
    filepath = os.path.join(checkpoint, model_name + '_states.pth')
    curpath = os.path.join(checkpoint, model_name + '_state_dict.pth')
    torch.save(state, filepath)
    torch.save(state['state_dict'], curpath)
    if is_best:
        filepath = os.path.join(checkpoint, model_name + '_states_best.pth')
        curpath = os.path.join(checkpoint, model_name + '_state_dict_best.pth')
        torch.save(state, filepath)
        torch.save(state['state_dict'], curpath)


def save_checkpoint(state, is_best, single=True, checkpoint='checkpoint', filename='checkpoint.pth.tar', sepoch=15):
    if single:
        fold = ''
    else:
    ...
    if is_best and state['epoch'] >= sepoch:
